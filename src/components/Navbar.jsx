    ...
    <>
      ...
      >
        ...
        <ul className="hidden md:flex text-[1.1vw] items-center gap-10">
          {menuItems.map((item) => (
            <li key={item} className="relative text-[1.6vw] lg:text-[1.2vw] group cursor-pointer">
              {item}
      ...
      >
        {/* Cross icon always visible over sliding menu */}
        ...
        <div className="flex flex-col justify-between py-6 items-center h-full gap-8">
          <div className="flex mt-[6vh] items-center flex-col">
            ...
            <ul className="flex flex-col items-center gap-6 text-xl font-semibold">
              {menuItems.map((item) => (
                <li key={item} className="cursor-pointer" onClick={() => setIsOpen(false)}>
                  {item}
                </li>
              ))}
            </ul>
            <button className="uppercase border bg-[#3E2519] text-white border-[#3E2519] rounded-lg px-6 py-3 mt-4">
              Free Consultation
            </button>
          </div>
